...
static key_mapping uiocode_keycode_table[] = {
    ...
    { .vcode = VC_COMPOSE,               .x11_key_name = "COMP" },
    { .vcode = VC_COMPOSE,               .x11_key_name = "MENU" },
    ...
    { .vcode = VC_COMPOSE,               .x11_key_name = "I135" },
...
for (unsigned int i = 0; i < sizeof(uiocode_keycode_table) / sizeof(uiocode_keycode_table[0]); i++) {
    if (keycode == uiocode_keycode_table[i].x11_keycode) {
            uiocode = uiocode_keycode_table[i].uiohook_key;
            break;
        }
    }
...
for (unsigned int i = 0; i < sizeof(uiocode_keycode_table) / sizeof(uiocode_keycode_table[0]); i++) {
    if (uiocode == uiocode_keycode_table[i].uiohook_key) {
            keycode = uiocode_keycode_table[i].x11_keycode;
            break;
        }
    }
