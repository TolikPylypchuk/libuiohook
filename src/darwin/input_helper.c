...
static const uint16_t uiocode_kvkcode_table[][2] = {
...
uint16_t keycode_to_uiocode(CGKeyCode keycode) {
    ...
    if (keycode < sizeof(uiocode_kvkcode_table) / sizeof(uiocode_kvkcode_table[0])) {
        uiocode = uiocode_kvkcode_table[keycode][0];
    }
...
CGKeyCode uiocode_to_keycode(uint16_t uiocode) {
    ...
    if (uiocode < 128) {
        keycode = (CGKeyCode) uiocode_kvkcode_table[uiocode][1];
    } else {
        // Calculate the upper offset.
        unsigned short i = (uiocode & 0x007F) | 0x80;

        if (i < sizeof(uiocode_kvkcode_table) / sizeof(uiocode_kvkcode_table[1])) {
            keycode = (CGKeyCode) uiocode_kvkcode_table[i][1];
        }
    }
